    ...
    class CreateSubtitles
    {
        public static bool CreateFileSubtitle(int index, List<string> texts)
        {
            try
            {
                ...
                subtitle.timeStart.milisecond = 200;

                subtitle.timeEnd = Process(index);
                subtitle.timeEnd.milisecond = 900;

                subtitle.contentSubtitle = texts;
                using (StreamWriter writer = new StreamWriter("data/Subtitles.txt", true))
                {
        ...
        static Time Process(int index)
        {
            ...
            {
                time.hour = 1;
                time.minute = index / 3600;
                time.second = index - (index / 3600) * 3600;
            }
            return time;
        }
    }
